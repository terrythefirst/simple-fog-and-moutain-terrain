...
void MyVulkanManager::initMatrix()
{
	...
	LightManager::setlightAmbient(0.35f, 0.35f, 0.35f, 1.0f);
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    uint8_t *pData2;
    result = vk::vkMapMemory(device, sqsCT->memUniformBuf2, 0, sqsCT->bufferByteCount2, 0, (void **)&pData2);
    assert(result == VK_SUCCESS);
    memcpy(pData2, uniformData, sqsCT->bufferByteCount2);
    vk::vkUnmapMemory(device, sqsCT->memUniformBuf2);
}
